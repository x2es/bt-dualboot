...
def require_bt_dir_access():
    invariant_and_halt(len(get_devices_paths()) == 0, f'No Bluetooth devices found!\nCheck if your user have access to {LINUX_BT_DIR} and at least one device paired. Try use sudo.')


def require_chntpw_package():
    invariant_and_halt(shutil.which('reged') == None, """ Missing dependency `reged`. Install `chntpw` package first. 
